...
export class DefaultStorageAssortmentHelper implements StorageAssortmentHelper {
		...
		await this.deleteAssortment.execute(
			{ id: assortment.id },
			{
				deleteProductImageByPath: async (path: string) =>
					await this.deleteFileByPath(S3FileStorageBucket.ASSORTMENT_IMAGES, path, currentUser),
				deleteQRCodeByPath: async (path: string) =>
					await this.deleteFileByPath(S3FileStorageBucket.QR_CODES, path, currentUser),
			},
			currentUser
		);
	}
		...
		const assortment = await this.createAssortment.execute(
			...
			},
			{
				...
				addAssortmentImageByBase64: async (path, base64) =>
					await this.uploadImageByBase64(S3FileStorageBucket.ASSORTMENT_IMAGES, path, base64, currentUser),
			},
			currentUser
		);
		assortments = await this.getAllAssortment.execute();
	...
	async importAndReplaceAssortment(dto: ImportAndReplaceAssortmentDTO, currentUser: UserDTO) {
		...
		const fullCurrentAssortments = await Promise.all(
			currentAssortments.map(async (assortment) => {
				// prettier-ignore
		...
		try {
			...
			await this.putUpAssortments(fullCurrentAssortments, currentUser);
