...
import { AssortmentHelper, Base64UploadFunction, DeleteProductImageByPathFunction, FileFetcher } from "../helpers/AssortmentHelper";
...
export class UpdateAssortment {
	...
	async execute(dto: UpdateAssortmentDTO, options: UpdateAssortmentOptions, currentUser?: UserDTO) {
		...
		const currentImage = assortment.image === null
			? null
			: await fetchAssortmentImage(assortment.image.id);
