...
export const assortment = async (
	...
	if (imageUrl) await ctx.remoteImage(
		imageUrl,
		startingX,
		startingY,
		{
	...
	if (index) {
		...
		ctx.document.text(
			index,
			afterImage,
			startingY + centeredInContainer(firstSectionHeight, heightOfIndexString),
		);
	}
	...
	ctx.document.fillColor(ctx.constants.colors.gray);
	ctx.document.fontSize(compact ? 10 : 12).text(
		contextString,
		afterImage,
		startingY + firstSectionHeight + (ctx.constants.margin / 2),
		{ width: lineWidth, align: "justify" },
	);
	ctx.document.fillColor(ctx.constants.colors.black);

	const qrCodeUrl = assortment.qrCode.visibility.publicUrl;
	if (qrCodeUrl) await ctx.remoteImage(
		qrCodeUrl,
...
export const assortments = async (
	ctx: SharedContext,
	assortments: AssortmentVO[],
	index?: string,
	height = 75,
) => {
	let i = 0;
